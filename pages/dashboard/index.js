...
import { POST_PRESENSI_DATANG, POST_UPSERT_PRESENSI_DATANG } from "../../gql/presensi";
import QrScanner from 'qr-scanner';

function CameraDatang({ isOpen }) {
    const video = useRef(null);
    const [qrScanner, setQrScanner] = useState();

    useEffect(() => {
        if (video.current && isOpen) {
            console.log("askdk")
        ...
        if (qrScanner && !isOpen) {
            console.log(qrScanner, "AAKKK")
            qrScanner?.stop();
            qrScanner?.destroy();
        }
        // eslint-disable-next-line
    }, [video.current, isOpen])
    return (
        <div>
...
function QRScanDatang({ dataSesi, dataTukang }) {
    ...
    async function handleScan(result) {
        try {
            ...
            console.log("a0a0a0a0", result)
            let tukang = dataTukang?.tukang?.find(x => x.id == tukangId);
            setLoadingAbsen(true)
            setLoadingAbsenNama("sedang mengabsen " + tukang.nama)
            ...
            if (res.data.insert_presensi.returning.length == 0) {
                toast.success(<Success text={`Sudah absen luwh: ${tukang.nama}`} />)
            } else {
    ...
    console.log(loadingAbsen, loadingAbsenNama, "aks")
            ...
            <ModalCamera title={"Scan untuk Datang"} closed={() => {
                ...
                <div>
                    ...
                    <div className="absolute bottom-0 right-0 z-50 w-8 h-8 border-b-8 border-r-8 border-red-500" /> */}
                        ...
                        <div className="bg-red-500 w-[w] h-full  scale-110 ">

                        </div>
                    </div> */}
                        {/* <CameraDatang isOpen={isOpen} /> */}
                        {/* 
                    {!loadingCamera ?
                        <div>
                            <QrReader
        ...
        <>
            ...
            >
                <div className="overflow-scroll h-[50vh]">
                    <div className=" py-4">
                        <h2 className="text-2xl font-semibold px-2 ">Presensi: Rabu, {dataSesiHariIni?.sesi[0] != undefined && dataSesiHariIni?.sesi[0]?.tanggal}</h2>
